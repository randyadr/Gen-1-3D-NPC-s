# GitHub auto-update setup — v1.1.5

This package is already configured for the public GitHub repository:

`randyadr/Gen-1-3D-NPC-s`

## First upload

1. Use the existing public GitHub repository `randyadr/Gen-1-3D-NPC-s`.
2. Upload **the contents of this folder** to the repository root.
3. Make sure the default branch is `main`.
4. Commit/push the files.
5. GitHub Actions runs `.github/workflows/release.yml` and publishes release `v1.1.5` with:
...
For the next mod update, change the mod files and bump `manifest.json` from `1.1.5` to `1.1.6`, then push to `main`. The workflow will create:

`gen1_true_3d_characters-1.1.6.zip`
...
After `v1.1.5` exists as a GitHub Release, submit the mod once to `bryanthaboi/gen1recomp-mod-index` with these values:

- id: `gen1_true_3d_characters`
- title: `Gen1 TRUE 3D Characters`
- version: `1.1.5`
- categories: `ART`, `CONTENT`
- github: `randyadr/Gen-1-3D-NPC-s`
- repo: `https://github.com/randyadr/Gen-1-3D-NPC-s`
