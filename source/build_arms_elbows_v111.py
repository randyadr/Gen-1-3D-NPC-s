...
BASE=Path('/mnt/data/gen1chars110')
OUT=Path('/mnt/data/Gen1True3DCharacters_v1.1.1')
ZIP=Path('/mnt/data/Gen1True3DCharacters_Gen1Recomp-v1.1.1.zip')
ASSETS=OUT/'assets'
...
def pose_params(theta=None):
    # Natural arm-chain pass. The source models arrive close to T pose, so the
    # upper arms are first lowered beside the torso; elbow and wrist motion are
    # then applied as child-joint rotations rather than rotating the limb as one
    # rigid stick.
    if theta is None:
        return dict(
            armL=0, armR=0,
            elL=math.radians(16), elR=math.radians(16),
            wrL=math.radians(-6), wrR=math.radians(-6),
            hipL=0, hipR=0, kneeL=0, kneeR=0, ankleL=0, ankleR=0,
        )

    s=math.sin(theta)
    # Arms swing opposite the legs. A slightly smaller shoulder arc combined
    # with a stronger elbow bend reads much more like a human walk at the tiny
    # overworld scale than straight pendulum arms.
    arm=math.radians(21)*s
    hip=math.radians(23)*s

    # The forward arm bends more at the elbow while the trailing arm opens a
    # little. 18..30 degrees is enough to make the elbow visibly articulate
    # without making the hands punch into the torso.
    elL=math.radians(18 + 12*(0.5 - 0.5*s))
    elR=math.radians(18 + 12*(0.5 + 0.5*s))

    # Wrists counter-rotate some of the forearm bend so hands continue the line
    # of the forearm instead of looking snapped or floppy.
    wrL=math.radians(-6 + 2.5*s) - elL*0.10
    wrR=math.radians(-6 - 2.5*s) - elR*0.10
